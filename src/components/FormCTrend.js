...
export const FormCTrend = (props) => {
    ...
    const buttonClick = () => {
        reset()
        if ((ins !== '') && (categoryValue !== '') && (seat_pool !== '') && (quotaValue !== '') && (trendIns !== 0)) {
            if ((rank !== '') && rank > '0') {
                setloading1(false)
                setbtn(true)
            } else {
    ...
    return (
        <>
            ...
            <h2 className="pageHeading">Past Trend of Opening/Closing Ranks for <span id="collegeId">all Branches</span> of <span id="collegeId">an Institute</span></h2>
            <Form>
                <div id="trendForm">
                    <div id="trendInput">
                        ...
                        <div id="trend1">
                            <label id="trendLabel1">Select Institute</label>
                            <Select
                                id="trendSelect"
                                name='Institute'
                                value={valueI}
                                                    ...
                                                    <Table.Row>
                                                        <Table.HeaderCell>
                                                            <div className="searchField">
                                                                Branch
                                                                <input type="text" className="mobileRemove" id="branch" placeholder="Search" onKeyUp={search} size={3} />
                                                            </div>
                                                        </Table.HeaderCell>
                                                        {yearArray.map(r => (
                                                            <Table.HeaderCell>
                                                        ...
                                                        <>
                                                            <Table.Row>
                                                                <Table.Cell id="trendColumn">
                                                                    {props.branches.find(o => o.id === parseInt(br)).branch_code}
                                                                </Table.Cell>
                                                                {yearArray.map(y => (
                                                                    <Table.Cell id="data" key={y}>
                                                                        {ranks[y].find(o => ((o.branch_code === parseInt(br)) && (o.institute_code === parseInt(trendIns)) && (o.quota === quotaValue) && (o.category === categoryValue) && (o.seat_pool === seat_pool))) === undefined ? '-' : <><p>{ranks[y].find(o => ((o.branch_code === parseInt(br)) && (o.institute_code === parseInt(trendIns)) && (o.quota === quotaValue) && (o.category === categoryValue) && (o.seat_pool === seat_pool))).opening_rank}</p><p>to</p><p>{ranks[y].find(o => ((o.branch_code === parseInt(br)) && (o.institute_code === parseInt(trendIns)) && (o.quota === quotaValue) && (o.category === categoryValue) && (o.seat_pool === seat_pool))).closing_rank}</p></>}
                                                                    </Table.Cell>
                                                                ))}
                                                            </Table.Row>
                                                        </>
